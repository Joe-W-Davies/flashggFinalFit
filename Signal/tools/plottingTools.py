...
def plotFTest(ssfs,_opt=1,_outdir='./',_extension='',_proc='',_cat='',_mass='125'):
  ...
  for k,ssf in ssfs.iteritems():
    ...
    hists[k].GetXaxis().SetTitle("m_{ee} [GeV]")
...
def plotInterpolation(_finalModel,_outdir='./',_massPoints='120,121,122,123,124,125,126,127,128,129,130'):
  ...
  haxes.GetXaxis().SetTitle("m_{ee} [GeV]")
  ...
  haxes.GetXaxis().SetRangeUser(110,140)
  haxes.Draw("AXIS")
...
def plotSignalModel(_hists,_opt,_outdir=".",offset=0.02):
  ...
  h_axes.GetXaxis().SetRangeUser(110,140)
  h_axes.SetTitle("")
  ...
  lat0.DrawLatex(0.225,0.91,"#bf{CMS} #it{%s}"%_opt.label)
  lat0.DrawLatex(0.77,0.91,"%s TeV"%(sqrts__.split("TeV")[0]))
  #lat0.DrawLatex(0.16+offset,0.83,"H#rightarrow#gamma#gamma")
  lat0.DrawLatex(0.16+offset,0.83,"H #rightarrow ee")
